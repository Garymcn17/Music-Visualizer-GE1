...
public class AudioScript : MonoBehaviour {
    ...
    public static float[] audioBand = new float[9];
    public static float[] audioBandBuffer = new float[9];
    ...
    void CreateAudioBands()
    {
        for (int i = 0; i < 8; i++)
        {
            if(bands[i] > _freqBandHighest[i])
            {
                _freqBandHighest[i] = bands[i];
            }
            audioBand[i] = (bands[i] / _freqBandHighest[i]);
        }
    }
